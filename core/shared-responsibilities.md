...
| `Monitor logs collected by logging agents`   | Provide images and instructions for how to install logging agents in environments that you want to monitor, such as Kubernetes, Linux, Openshift. | [Install and configure logging agents](/docs/Log-Analysis-with-LogDNA?topic=Log-Analysis-with-LogDNA-config_agent_kube_cluster). </br>Monitor that the agents are running in your environment. |
...
| `Recovery of logging agents`                                     | `N/A` | [Reinstall](/docs/Log-Analysis-with-LogDNA?topic=Log-Analysis-with-LogDNA-log_analysis_agent#log_analysis_agent_configure) the logging agent in the event of any disaster event that impacts the agent runtime. |
